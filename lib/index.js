...
module.exports = class SpikeDatoCMS {
  ...
  apply (compiler) {
    ...
    // preparing for single page template parsing
    const templatePairs = this.models.reduce((m, model, idx) => {
      if (!model.template) return m
      if (!model.template.path) {
        throw new Error(`${model.name}.template must have a "path" property`)
      }
      if (!model.template.output || typeof model.template.output !== 'function') {
    ...
    compiler.plugin('before-loader-process', (ctx, options) => {
      ...
      if (!Array.isArray(data)) {
        throw new Error(`${conf.name}'s must return an array in order to render templates`)
      }
