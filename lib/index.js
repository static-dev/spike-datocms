...
module.exports = class SpikeDatoCMS {
  ...
  apply(compiler) {
    ...
    compiler.plugin('before-loader-process', (ctx, options) => {
      ...
      const data =
        this.addDataTo.dato[conf.name] || this.addDataTo.dato[conf.type]
    ...
    return W.all([
      ...
      W.reduce(
        ...
        (memo, model) => {
          ...
          const options = {
            allPages: true,
            version: this.drafts ? 'latest' : 'published'
          }
          if (model.ids) {
            options['filter[ids]'] = model.ids
          }
          if (model.query) {
            options['filter[query]'] = model.query
          }
          if (model.offset) {
            options['page[offset]'] = model.offset
          }
          if (model.limit) {
            options['page[limit]'] = model.limit
          }
          ...
          return (
            W.all([
              ...
              .then(([records, itemTypes]) => {
                ...
                if (modelRecords.length && modelRecords[0].position) {
                  modelRecords = modelRecords.sort(
      ...
      if (this.addDataTo.dato) {
        delete this.addDataTo.dato
      }
      Object.assign(this.addDataTo, {
        dato: Object.assign(models, { _meta: site })
      })
      done()
    }, done)
  }
